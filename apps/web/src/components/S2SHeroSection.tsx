...
import { type FinanceData } from "../services/api";
...
export function S2SHeroSection({ financeData }: S2SHeroSectionProps) {
  const [period, setPeriod] = useState<S2SPeriod>("month");
  const [showBreakdown, setShowBreakdown] = useState(false);

  // Map real data to S2S concept
  const income = financeData?.total_income || 0;
  const fixed = financeData?.total_expense || 0;
  const savings = financeData?.goals_allocation || 0;
  const buffer = financeData?.emergency_buffer || 0;
  
  // Safe-to-Spend logic
  const s2sBudget = income - fixed - savings - buffer;
  const remaining = financeData?.safe_to_spend || 0;
  const spent = s2sBudget - remaining;
  
  const percent = s2sBudget > 0 ? Math.min(Math.round((spent / s2sBudget) * 100), 100) : 0;
  
  const status: S2SStatus = percent > 90 ? "danger" : percent > 70 ? "warning" : "safe";
  ...
  const displayRemaining = period === "month" ? remaining : remaining / (period === "today" ? 30 : 4); // Simplified mock period diff
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[12px] font-semibold" style={{ color: "rgba(255,255,255,0.28)" }}>
                Hệ thống thời gian thực · Ngày nhận lương: {financeData?.income_date || 1} hàng tháng
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center gap-8 lg:gap-12">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <Pill label={formatVND(income)} sub="Thu nhập" />
              <span className="text-[14px] font-bold" style={{ color: "rgba(255,255,255,0.25)" }}>−</span>
              <Pill label={formatVND(fixed)} sub="Chi cố định" />
              <span className="text-[14px] font-bold" style={{ color: "rgba(255,255,255,0.25)" }}>−</span>
              <Pill label={formatVND(savings)} sub="Tiết kiệm" />
              <span className="text-[14px] font-bold" style={{ color: "rgba(255,255,255,0.25)" }}>−</span>
              <Pill label={formatVND(buffer)} sub="Buffer" />
              <span className="text-[14px] font-bold" style={{ color: "rgba(255,255,255,0.25)" }}>=</span>
              <Pill label={formatVND(s2sBudget)} sub="Budget S2S" accent />
            ...
            {period === "month" && (
              ...
              >
                ...
                <span className="text-[10px] font-semibold" style={{ color: "rgba(255,255,255,0.4)" }}>
                  Tiêu hao trung bình: {formatVND(Math.round(spent / 30))} / ngày
                </span>
              </div>
            )}
          </div>

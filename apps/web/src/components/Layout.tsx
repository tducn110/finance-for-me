...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  // @ts-ignore - Handle possible partial user object
  const displayUser = user || { fullName: "Khách", username: "guest", avatarUrl: null, avatarText: "?" };
  const userInitials = displayUser.avatarText || (displayUser.fullName ? displayUser.fullName.split(" ").map((n: string) => n[0]).join("").substring(0, 2).toUpperCase() : "?");
        ...
        <div className="p-4 mt-auto">
          <div className={`p-3 rounded-2xl bg-gradient-to-br from-gray-50 to-gray-100/60 border border-gray-200/50 flex items-center gap-3 shadow-sm ${!sidebarOpen && "justify-center"}`}>
            <motion.div 
              ...
              className="w-9 h-9 rounded-full bg-gradient-to-br from-blue-400 to-indigo-500 border border-blue-300/50 flex items-center justify-center text-white text-sm font-bold flex-shrink-0 shadow-sm overflow-hidden"
            >
            ...
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                <p className="text-[13px] font-bold text-gray-900 truncate">{displayUser.fullName}</p>
                <p className="text-[11px] font-medium text-gray-500 truncate">@{displayUser.username}</p>
              </div>
            )}
          </div>
          <button
      ...
      <div className={`flex-1 flex flex-col transition-all duration-300 relative z-10 ${sidebarOpen ? "md:ml-[260px]" : "md:ml-[80px]"}`}>
        ...
        <header className="sticky top-0 z-30 bg-white/60 backdrop-blur-2xl border-b border-gray-200/40 px-6 lg:px-10 h-[80px] flex items-center gap-6 shadow-sm">
          ...
          <div className="flex-1">
            ...
            >
              {getGreeting()}, {displayUser.fullName.split(" ").pop()} 👋
            </motion.h1>
